...
int SquareMaking::getMinimalPrice(int a, int b, int c, int d) {
	int avg = (a+b+c+d)/4;
	int min_dist = abs(avg-a);
	int min_elem = a;
	if(min_dist>abs(avg-b)){
		min_dist = abs(avg-b);
		min_elem = b;
	}
	if(min_dist>abs(avg-c)){
		min_dist = abs(avg-c);
		min_elem = c;
	}
	if(min_dist>abs(avg-d)){
		min_dist = abs(avg-d);
		min_elem = d;
	}
	return(abs(min_elem-a)+abs(min_elem-b)+abs(min_elem-c)+abs(min_elem-d));
}
